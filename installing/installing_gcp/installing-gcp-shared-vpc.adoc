...
[IMPORTANT]
====
In a shared VPC environment, the component service accounts that the `ccoctl` utility creates require additional permissions in the host project to access shared VPC network resources. Ensure that you grant the required host project permissions to these service accounts after running the `ccoctl` utility. For more information, see xref:../../installing/installing_gcp/installing-gcp-account.adoc#minimum-required-permissions-ipi-gcp-xpn-short-term-creds_installing-gcp-account[Additional host project permissions for {gcp-short} Workload Identity].
====
